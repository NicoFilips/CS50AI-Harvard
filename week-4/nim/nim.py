...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        """
        # Convert state to tuple for immutability and dictionary key compatibility
        state_action_pair = (tuple(state), action)
    
        # Calculate the old Q-value or initialize it to 0 if not previously encountered
    ...
    def choose_action(self, state, epsilon=True):
        ...
        max_reward = float('-inf')  # Initialize to negative infinity to handle negative Q-values.
        best_action = None
        
        # Evaluate each possible move
        for move in available_moves:
            q = self.q.get((tuple(state), move), 0)  # Use get with default value to simplify handling missing keys
            if q > max_reward:
                max_reward = q
                best_action = move
